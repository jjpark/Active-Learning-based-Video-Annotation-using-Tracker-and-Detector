...
if( exist(crDir,'dir')&stage==1)
  % if window directory is specified simply load windows
  disp('direct load from WinDir');
  fs=alQueries('mulGetFilesReversed',{crDir}); nImg=length(fs); assert(nImg>0);
  if(nImg>n), fs=fs(:,randSample(nImg,n)); end; n=nImg;
  disp(n);
  for i=1:n, if(rem(i,1000)==0);disp(i);end;fs{i}=[{opts.imreadf},fs(i),opts.imreadp]; end
  Is=cell(1,n); parfor i=1:n, if(rem(i,1000)==1);disp(i);end;Is{i}=feval(fs{i}{:}); end
else
  ...
  fs=alQueries('mulGetFilesReversed',fs); nImg=size(fs,2); assert(nImg>0);
